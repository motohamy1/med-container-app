...
const TARGET_MIN_TOPICS = 52; // Target 50+ topics per category

const SPECIALTY_META = {
  // --- MEDICAL DISCIPLINES ---
  nephrology: {
    name: 'Nephrology & Renal Medicine',
    scope: 'Renal physiology, acute kidney injury, chronic kidney disease, dialysis modalities (HD/PD/CRRT), glomerular diseases, tubulointerstitial nephritis, electrolyte and acid-base emergencies, renal transplantation, hypertension, and KDIGO guidelines.'
  },
  endocrinology: {
...
const CATEGORY_META = {
  emergencies: {
    name: 'Emergencies & Resuscitation',
    desc: 'Acute life-threatening presentations, emergency dosing, resuscitation algorithms, and critical stabilization protocols'
  },
  clinical_topics: {
    name: 'Clinical Topics & Guidelines',
    desc: 'Standard clinical management, outpatient and inpatient diagnostic workups, core disease pathophysiology, and definitive society guidelines'
  },
  tools: {
    name: 'Tools, Diagnostics & Scoring Systems',
    desc: 'Clinical risk scores, prognostic calculators, diagnostic algorithms, staging systems, and procedural guidance'
  },
  research: {
    name: 'Recent Research & Landmark Trials',
    desc: 'Breakthrough clinical trials, recent FDA approvals, prospective studies, and evidence-based paradigm shifts'
  }
};
...
async function populateCategory(specialtyId, categoryId) {
  const spec = SPECIALTY_META[specialtyId];
  ...
  console.log(`\n======================================================================`);
  console.log(`🚀 [${spec.name} -> ${cat.name}] Starting ingestion: ${currentCount} -> ${TARGET_MIN_TOPICS}`);
  console.log(`======================================================================`);
  ...
  const systemPrompt = `You are a Chief Clinical & Surgical Specialist & Medical Textbook Editor in ${spec.name}.
Output pure JSON with the exact format:
{
  "topics": [
    {
      "id": "slug_topic_id",
      "title": "Topic Title",
      "subtitle": "Clinical/Surgical Subtitle with Key Interventions / Drugs / Landmark Trials / Diagnostic Tools",
      ...
      "clinical_content": [
        ...
        { "title": "Exact Reference & Guideline Citations", "content": "Official society guidelines (KDIGO, ADA, AAP, NCCN, ASCO, ACR, EULAR, APA, AAO, ACS, WSES, EAST, STS, SVS) and landmark trials with years" }
      ]
    }
  ]
}`;
  ...
  while (currentCount < TARGET_MIN_TOPICS) {
    ...
    console.log(`   ⚡ [${specialtyId}/${categoryId}] Generating batch of ${batchSize} clinical topics (Remaining needed: ${needed})...`);

    const userPrompt = `Generate exactly ${batchSize} DISTINCT, HIGH-YIELD clinical/surgical topics for:
...
Every topic must have real clinical rigor, exact dosages (e.g., mg/kg, IV rates, oral doses), operative surgical steps/indications, scoring criteria, and peer-reviewed guideline citations.`;

    const generated = await callAIWithFallback(systemPrompt, userPrompt);

    if (!generated || generated.length === 0) {
      console.warn(`   ⚠️ No topics generated in this turn, pausing 2s before retry...`);
...
async function runMasterExpansionAll() {
  console.log("======================================================================");
  console.log("🏥 MASS EXPANSION OF ALL MEDICAL & SURGICAL SPECIALTIES (50+ TOPICS)");
  console.log("======================================================================");
  ...
  console.log(`Total category tasks to process: ${taskQueue.length} across ${specialties.length} disciplines.`);

  // Process 2 categories in parallel for optimal throughput and rate limit safety
  await asyncPool(2, taskQueue, async ({ specId, catId }) => {
    await populateCategory(specId, catId);
  });

  console.log("\n======================================================================");
  console.log("🎉 ALL MEDICAL & SURGICAL SPECIALTIES FULLY INGESTED (>= 50 TOPICS EACH)!");
  console.log("======================================================================");
}

runMasterExpansionAll();
